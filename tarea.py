...
# Función para consultar el fabricante de una dirección MAC usando una API externa
def consultar_mac(direccion_mac):
    ...
    try:
        ...
        if respuesta.status_code == 200:
            datos = respuesta.json()  # Convertimos la respuesta a JSON para manipular los datos
        ...
        # En caso de que haya un error durante la solicitud, mostramos un mensaje amigable
        print(f"Error en la solicitud: {error}")

# Función para mostrar la tabla ARP en Windows y consultar los fabricantes de las MACs encontradas
def mostrar_arp():
    try:
        # Ejecutamos el comando 'arp -a' usando subprocess para obtener la tabla ARP de la red local
        resultado = subprocess.check_output("arp -a", shell=True, text=True)
        
        # Dividimos el resultado en líneas para procesarlo más fácilmente
        lineas = resultado.splitlines()

        # Recorremos línea por línea el resultado de la tabla ARP
        for linea in lineas:
            partes = linea.split()  # Dividimos cada línea en partes usando espacios como separador
...
def mostrar_ayuda():
    ...
    print("--mac: Consulta el fabricante de una dirección MAC (ejemplo: aa:bb:cc:00:00:00)")
    print("--arp: Muestra los fabricantes de los dispositivos en la tabla ARP")
    print("--help: Muestra este mensaje de ayuda")

# Función principal que maneja los argumentos proporcionados por el usuario
def main(argv):
    try:
    ...
    direccion_mac = "B4:B5:2F:1B:FF"  # Esta variable guardará la dirección MAC si se proporciona
